...
public class VaccineTypeController {	
	...
	@PostMapping("/edit")
	public String edit(@ModelAttribute("vaccineType") VaccineType vaccineType,  Model model, @RequestParam(value="vaccineTypeSubmit") String vaccineTypeSubmit) {
		
		try {
			String name = vaccineType.getName(),
				   currentUser = "Mohammed Monirul Islam",
	...
	public String delete(@PathVariable("name") String name,  Model model) {
		...
		try {
			...
			model.addAttribute("success", messageSource.getMessage("vaccineTypeDeleteSuccess", new String[] {name}, locale));
		} catch (Exception e) {
			e.printStackTrace();
			model.addAttribute("error", messageSource.getMessage("error", new String[] {e.getMessage()}, locale));
		}
